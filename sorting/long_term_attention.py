...
from basis_functions import (CosineBasisFunctions, GaussianBasisFunctions,
                             PowerBasisFunctions, SineBasisFunctions)
...
class LongTermAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.proj_query = nn.Linear(
            n_heads * head_size, n_heads * head_size, bias=False
        )
        ...
        if self.mask:
            if self.mask_type == "cnn":
                self.mask_net = torch.nn.Conv1d(
                    n_heads * head_size, n_heads * head_size, 3, padding=1
                )
        ...
        for l in lengths:
            ...
            self.add_gaussian_basis_functions(
                self.psi[l], attn_num_basis, sigmas, device=self.device
            )
            ...
            if self.spacing == "linear":
                if padding:
                    if l % 2:
                        ...
                        positions = torch.linspace(
                            -0.5 + shift, 1.5 - shift, 2 * l - 1
                        ).to(self.device)
                    else:
                        shift = 1 / float(2 * l)
                        positions = torch.linspace(-0.5 + shift, 1.5 - shift, 2 * l).to(
                            self.device
                        )
                else:
                    shift = 1 / float(2 * l)
                ...
                if padding:
                    ...
                    pos = (
                        np.e ** (np.log(1 + 1) * torch.arange(1, length + 1) / length)
                        - 1
                    )
                    positions = torch.cat(
                        [
            ...
            self.Gs[l] = compute_G(
                l, self.psi[l][0], positions, padding=padding
            )  # [L,N]
        ...
        if self.infinite_memory:
            ...
            tm_l = torch.arange(
                self.nb_samples + 1, length + self.nb_samples + 1
            ).float()
            tm_tau = tm_tau * self.tau / self.nb_samples  # positions of old vectors
            tm_l = (
                self.tau + (1 - self.tau) * (tm_l - self.nb_samples) / length
            )  # positions of new vectors
            ...
            if padding:
                if l % 2:
                    ...
                    positions_pad_ = torch.linspace(
                        -0.5 + shift, 0, 2 * (length + self.nb_samples) - 1
                    ).to(self.device)
                else:
                    shift = 1 / float(2 * length + self.nb_samples)
                    positions_pad = torch.linspace(
                        -0.5 + shift, 1.5 - shift, 2 * (length + self.nb_samples)
                    ).to(self.device)
                positions_pad_ = torch.FloatTensor(
                    [i for i in positions_pad if i < 0]
                ).to(self.device)
                positions_pad__ = torch.FloatTensor(
                    [i for i in positions_pad if i > 1]
                ).to(self.device)
                positions_inf = torch.cat(
                    [positions_pad_, positions_inf, positions_pad__], dim=0
                )
            ...
            self.G_inf = compute_G(
                self.nb_samples + length,
                self.psi[length][0],
                positions_inf,
                padding=padding,
            )  # [L+nb_samples,N]
    ...
    def update_inf(self, x, layer_n=None, doc_final=False):
        if self.B_past is not None:
            xm_tau = self.B_past.transpose(-1, -2).matmul(
                self.samples.transpose(0, 1)
            )  # [B,e,nb_samples]

            x = torch.cat([xm_tau, x], dim=2)  # [B,e,nb_samples+L]
            B = self.value_function(
                x, inf=True, layer_n=layer_n, doc_final=doc_final
            )  # [B,N,e]
        else:
    ...
    def forward(self, k, q, new_doc=False, layer_n=0, reg_mask=None, doc_final=False):
        ...
        if self.infinite_memory:
            ...
            B = self.value_function(
                k.view(klen, batch_size, -1, layer_n=layer_n, doc_final=doc_final)
            )  # [B,N,e]
        ...
        query = query.view(batch_size, qlen, self.n_head, self.d_head).transpose(
            1, 2
        )  # [B,h,q,d]
        keys = keys.view(
            batch_size, self.attn_num_basis, self.n_head, self.d_head
        ).transpose(
            1, 2
        )  # [B,h,N,d]
        values = values.view(
            ...
            if self.mu_0 is None:
                kl_reg = (
                    ...
                    * (
                        ...
                        - torch.log(sigma_sq.view(batch_size, -1) / sigma_0_sq)
                        - 1
                    )
                )
            else:
        ...
        theta = torch.zeros(
            batch_size * self.n_head * qlen, 2, device=self.device
        )  # [B*h*q, 2]
        theta[:, 0] = mu / sigma_sq
        theta[:, 1] = -1.0 / (2.0 * sigma_sq)
        ...
        r = r.view(batch_size, self.n_head, qlen, self.attn_num_basis).permute(
            0, 1, 3, 2
        )  # [B,h,N,q]
        ...
        context = context.contiguous().view(
            qlen, batch_size, self.n_head * self.d_head
        )  # [q,B,e]
